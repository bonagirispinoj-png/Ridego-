...
<div id="screen-user-booking" class="screen page-with-nav">
  ...
  <div class="card" style="margin-top:0">
    ...
    <div class="location-input-wrap">
      ...
      <span class="loc-icon" onclick="App.calcRoute()">🗺</span>
    </div>
...
<div id="screen-user-rides" class="screen page-with-nav">
  <div class="app-header"><div style="font-size:18px;font-weight:800;color:#fff">My Rides</div></div>
  <div id="user-rides-list" style="padding-top:8px"></div>
</div>

<!-- ============================== USER PROFILE ============================== -->
<div id="screen-user-profile" class="screen page-with-nav">
  <div class="app-header"><div style="font-size:18px;font-weight:800;color:#fff">My Profile</div></div>
...
async function drawLeafletRoute(pLat, pLng, dLat, dLng) {
  ...
  try {
    ...
    if (data.routes && data.routes[0]) {
      ...
      bookingPolyline = L.polyline(coords, { color: '#FF6B35', weight: 5, opacity: 0.85 }).addTo(bookingMap);
  ...
  // fallback haversine
  const km = haversineKm(pLat, pLng, dLat, dLng) * 1.25;
  const mins = Math.ceil(km * 3);
  return { km, mins, distText: '~' + km.toFixed(1) + ' km', timeText: '~' + mins + ' min' };
}
...
window.App = {
  ...
  pickupInputChanged: async function() {
    ...
    sugEl.innerHTML = results.map(p =>
      `<div class="suggestion-item" onclick="App.selectPickupSugg('${encodeURIComponent(p.display_name)}','${p.lat}','${p.lon}')">📍 ${p.display_name.substring(0,60)}</div>`
    ).join("");
  },
  ...
  dropInputChanged: async function() {
    ...
    sug.innerHTML = results.map(p =>
      `<div class="suggestion-item" onclick="App.selectDropSugg('${encodeURIComponent(p.display_name)}','${p.lat}','${p.lon}')">📍 ${p.display_name.substring(0,60)}</div>`
    ).join("");
  },
  ...
  listenForAvailableRides: async function() {
    ...
    // Check for overdue fees first — block from getting new orders
    const hasOverdueFees = await checkDriverPendingFees();
    if (hasOverdueFees) {
      const container = document.getElementById("driver-available-rides");
      if (container) container.innerHTML = `<div class="fee-warning" style="margin:10px"><h4>⚠️ Platform Fee Overdue</h4><p>Pay your overdue platform fees to start receiving orders again.</p><button class="btn btn-primary btn-sm" style="margin-top:8px" onclick="App.driverNav('fees')">💳 Pay Platform Fees</button></div>`;
      return;
    }
    ...
    state.driverRidesSub = onSnapshot(q, snap => {
      ...
      if (!rides.length) {
        container.innerHTML = `<div style="text-align:center;padding:20px;color:var(--muted);font-size:14px">🔍 No pending rides for ${vehicle} right now...</div>`;
        return;
      }
    ...
    snap.forEach(d => {
      ...
      html += `<div class="card">
        ...
        <div style="display:flex;gap:6px;margin-top:8px;flex-wrap:wrap">
          ...
          <a href="https://wa.me/91${u.phone}" class="btn btn-sm btn-gray" target="_blank">📱 WhatsApp</a>
          <button class="btn btn-sm btn-green" onclick="App.adminApproveDriver('${d.id}')">✅ Approve</button>
...
<!--
════════════════════════════════════════════════════
  RIDEGO v4 — COMPLETE SETUP GUIDE
════════════════════════════════════════════════════

MAPS: Uses Leaflet (OpenStreetMap) + OSRM routing — NO API KEY NEEDED!
      Routes, distances, and fares work out of the box.

FIREBASE SETUP:
  1. Enable Email/Password Auth
  2. Create admin: bonagirispinoj@gmail.com
  3. Apply Firestore Rules below
...
NEW FEATURES in v4:
✅ Leaflet maps (OpenStreetMap) — NO API KEY needed, works immediately!
✅ OSRM routing — real road routes, real distances, real fare calculation
✅ QR code payment embedded (Spinoj Bonagiri UPI)
✅ Cash ride → Driver pays 20% platform fee within 24hrs rule
✅ Platform fee tracking — drivers blocked from orders if overdue
✅ Admin platform fee management panel
✅ Driver approval WITHOUT documents — admin can approve anyone
✅ Driver sees user phone & location after accepting
✅ User sees driver phone & location after driver accepts
✅ Driver map with Google Maps navigation button
✅ Live GPS tracking updates ride doc in real-time
════════════════════════════════════════════════════
-->
